...
## 6. Розділ «Безпека» (Загрози та заходи протидії)

1. **Несанкціонований фізичний або бездротовий доступ до вузлів (Node Tampering / Spoofing)**
   * *Загроза:* Зловмисник може підключитися до бездротової мережі або фізично підмінити датчик для спотворення показників мікроклімату.
   * *Захід протидії:* Використання криптографічної автентифікації пристроїв на основі сертифікатів (X.509) та захищених протоколів зв'язку (WPA3 для Wi-Fi, шифрування AES для Zigbee).
2. **Перехоплення та модифікація даних у мережі (Man-in-the-Middle, MitM)**
   * *Загроза:* Перехоплення керуючих команд або даних телеметрії на шляху від датчиків до шлюзу чи хмари.
   * *Захід протидії:* Застосування захищеного протоколу MQTT over TLS (MQTTS) з шифруванням трафіку на транспортному рівні.
3. **Кібератака на хмарний сервер / відмова в обслуговуванні (DDoS)**
   * *Загроза:* Втрата контролю над теплицею через недоступність хмари внаслідок DDoS-атаки.
   * *Захід протидії:* Реалізація автономного режиму роботи (Edge autonomy) та заборона блокування роботи локальних виконавчих механізмів при відсутності зовнішнього сигналу (Fail-Safe логіка).

---

## 7. Джерела

* ITU-T Y.2060: *Overview of the Internet of Things* (Дата звернення: 12.09.2026).
* NISTIR 8259: *Foundational Cybersecurity Activities for IoT Device Manufacturers* (Дата звернення: 12.09.2026).
* ENISA: *Baseline Security Recommendations for IoT* (Дата звернення: 12.09.2026).
* MQTT Official Specification: *OASIS Standard* (Дата звернення: 12.09.2026).
